...
func (s *RenderSystem) Draw(screen *ebiten.Image, em *EntityManager, assetManager *AssetManager) {
	...
	for _, entityID := range entities {
		...
		if image != nil {
			...
			// Check if this is a player entity and handle facing direction
			if em.HasComponent(entityID, ComponentTypePlayer) {
				playerComp, _ := em.GetComponent(entityID, ComponentTypePlayer)
				player := playerComp.(*PlayerComponent)
				
				if !player.FacingRight {
					// Flip horizontally when facing left
					// Scale by -1 on X axis and translate to correct position
					options.GeoM.Scale(-1, 1)
					options.GeoM.Translate(float64(sprite.Width), 0)
				}
			}
